...
static off_t package_extract_self_parse_header_length (const char *inh, size_t len)
{
  char lintbuf[15];
  long r;
  char *endp;
...
static int package_extract_self (int fd)
{
  ...
  while (1)
  {
    ...
    off_t memberlen = package_extract_self_parse_header_length (arh.ar_size, sizeof (arh.ar_size));
    if (memberlen <= 0)
      return 1;
