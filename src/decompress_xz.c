...
struct decompress_xz *decompress_xz_new (int fd, off_t len)
{
  struct decompress_xz *c = calloc (1, sizeof (struct decompress_xz));
  c->fd = fd;
  c->len = len;

  log_text (DI_LOG_LEVEL_DEBUG, "XZ decompression: New. Len: %lld", (long long) len);
...
ssize_t decompress_xz (struct decompress_xz *c, int fd)
{
  ...
  do
  {
    ...
    ret = lzma_code (&c->stream, LZMA_RUN);
