...
int gpg_init (const char **_keyringdirs, const char *keyring_name)
{
  ...
  if (!keyring_name && suites_suite)
    keyring_name = suites_suite->keyring;
  
  if (!keyring_name)
    log_text (keyring_level, "Have no keyring");
  else if (strstr (keyring_name, "/"))
  {
    struct stat s;
